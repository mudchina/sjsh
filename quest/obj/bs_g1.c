...
void create()
{
	...
	if( clonep() )
		...
		set("unit","颗");
                set("color","g");  //绿
                set("value", 40000);
		set("long",HIG"一块闪闪发光的宝石，据说它的上边吸附着神奇的力量。\n"NOR);
	}
	setup();
}
...
int do_enchase(string arg)
{
   ...
   if(!in_obj->query("weapon_prop/damage"))   //防具
  {
       ...
       tell_room(environment(me),HIG"只见"+ob->name()+HIG"发出一团耀眼的金光,环绕在"+in_obj->query("name")+HIG"的周围 \n"NOR);
     ...
     //in_obj->set("no_sell",1);       
       ...
       message_vision(CYN"$N把$n"+CYN"镶嵌到"+in_obj->query("name")+CYN"上 \n"NOR,me,ob);
       tell_room(environment(me),HIG"只见"+ob->name()+HIG"发出一团耀眼的金光光,环绕在"+in_obj->query("name")+HIG"的周围 \n"NOR);
