...
int perform(object me, object target)
{
	...
	extra = me->query_skill("dashen-bian",1) ;
	if ( extra < 100) return notify_fail("你的「打神鞭法」还不够纯熟！\n");
		
	if( !target ) target = offensive_target(me);
	if( !target
	||	!target->is_character()
	||	!me->is_fighting(target) )
		return notify_fail("「驱神逐魔」只能对战斗中的对手使用。\n");
...
if(random(hisexp) < myexp*2/3 && weapon)   
	{	message_vision(
HIB "\n\n$N使出一式打神鞭法中的「驱神逐魔」，手中"+(me->query_temp("weapon"))->name()+"将$n的兵刃越缠越紧，$n一个把持不住，手中"+ 
weapon->name()+  "脱手而飞！\n\n" NOR, me,target);
     weapon->unequip();
	...
	{
message_vision(
HIB "\n\n$N使出一式打神鞭法中的「驱神逐魔」，手中"+(me->query_temp("weapon"))->name()+  "将$n越缠越紧，逼的$n一阵手忙脚乱！\n\n" NOR, me,target);
