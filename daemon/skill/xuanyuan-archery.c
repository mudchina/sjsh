...
int valid_learn(object me)
{
  ...
  if( !(ob=me->query_temp("weapon")) || (string)ob->query("apply/skill_type") != "archery" )
...
{
  return CLASS_D("fighter") + "/xuanyuan-archery/" + action;
}
...
int practice_skill(object me)
{
  ...
  if( !objectp(weapon = me->query_temp("weapon"))
     ||      (string)weapon->query("apply/skill_type") != "archery" )
...
void name_skill(object me, string arg)
{
   ...
   int content;
   string msg;
m_actions=me->query("skill_xuanyuan-archery");
   ...
   if(!arg||arg==" ")arg="轩辕箭法必杀技"+chinese_number(sizeof(m_actions)+1);
   m_act+=(["name":arg]);
   m_act+=(["action":msg]);
   m_actions+=({m_act});
   me->set("skill_xuanyuan-archery",m_actions);
}
