...
void autosave()
{
 ...
 int i;
        user = users();
        message("channel:chat",HIR"〖太平盛世〗："HIY"自动更新三界神话任务榜。\n"NOR,users());
...
object xu=load_object("/d/huanggong/npc/dachen2");
        ...
        dirs = ({"/d/gao/","/d/jz/", "/d/changan/","/d/eastway/","/d/kaifeng/",
         ...
         n =  (int)(sizeof(users())/7+random(sizeof(users())/7)+1);
       message("system",HIW "【黑暗天神】:"+HIR+"魔界杀手们听令！血洗那个所谓天神真身所创造的三界神话世界！\n"NOR, users());
        for(x=0;x<n;x++)
        {
 emery=new("/u/stey/npc/pker.c");
        emery->random_go(me,dirs,i);
        }
message("system",GRN"【天神真身】：现共有"+chinese_number(n)+"个魔界杀手正在"+TASK_D->query_city(environment(find_living("sha shou")))+"作乱！\n"NOR,users());
        xu->set("mark",time());
        remove_call_out("check_guaiwu");
        call_out("check_guaiwu",10,xu);
...
void check_guaiwu(object xu)
{
      ...
      if(sizeof(filter_array(children(FILE),(:clonep:))) > 1)
       {      
     message("system",GRN"【天神真身】：大家加油啊，魔界杀手还没有杀完！\n"NOR,users());
        remove_call_out("check_guaiwu");
        call_out("check_guaiwu",20,xu);
        }
        else 
        {
  message("system",GRN"【天神真身】：恭喜各位杀死所有的魔界杀手，感谢大家保护了三界神话世界！！\n",users());
  remove_call_out("reward");
        call_out("reward",3);
       }
