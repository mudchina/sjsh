...
mixed ask_for_qob(object npc2, object qob, string qob_name)
{
        ...
        {
                message_vision("$N瞥了$n一眼，咳嗽两声，对着"
                               "$n就聊了开来。\n", me, npc2);
                tell_object(me, CYN "你劝说" + npc2->name() +
                                CYN "将" + qob_name +
                                CYN "交出来，大家都了事。\n" NOR);
                if (me->query_int() > random(40))
                {
                        ...
                        {
                                message_vision("$N叹了口气，对$n说：“你就"
                                               "别说了，那东西真的不在我身"
                                               "上了！”\n", npc2, me);
                        }
                } else
                        message_vision("$N眼睛一瞪，对着$n骂了两"
                                       "声，道：“滚！少来废话！”\n",
                                       npc2, me);
                return 1;
        }
...
void do_say(object knower)
{
        ...
        message_vision("$N又自言自语道：结果你争我夺的，这不，"
                       "果然就没什么好下场！\n", knower);
}
...
int npc_accept_object(object me, object who, object ob)
{
        ...
        if (ob != QOB)
        {
                message_vision("$N瞪着$n，看了半天才道：“这是什么？”\n",
                               me, who);
                return 0;
        }

        message_vision("$N大喜过望，对$n道：“这位" +
                       RANK_D->query_respect(who) +
                       "！太感谢了，" + RANK_D->query_self(me) +
                       "实在不知道该怎么报答你！”\n", me, who);
        reward = new("/clone/money/gold");
        reward->set_amount(2);
        message_vision("$N掏出" + reward->short() +
                       "，双手奉上，感激道：“一点薄礼，不"
                       "成敬意，不成敬意！”\n", me, who);
        reward->move(who, 1);
