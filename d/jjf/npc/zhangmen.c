...
int rank_me()
{
	...
	exp=exp+dx;
        if( me->query("title") == "大唐天下兵马大元帅")
		return notify_fail("你已经是掌门身份，还来要什么职位．\n");
	if( me->query("family/family_name") == "将军府")
	{
                if( exp <= 40000 ){
			me->set("title", "将军府后备役新兵");
		} else if(exp <= 100000 ){
			me->set("title", "将军府前锋营参将");
		} else if(exp < 200000 ){
			me->set("title", "将军府神机营带刀校尉");
		} else if(exp <= 400000 ){
			me->set("title", "将军府前锋营统领");
		} else if(exp <= 1000000 ){
			me->set("title", "将军府铁骑营神风骑士");
		} else if(exp <= 2000000 ){
			me->set("title", "将军府一品金甲锦衣卫");
		} else if(exp <= 4000000 ){
		     /*  if ( exp > exp4){
