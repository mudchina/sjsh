...
void create()
{
   set_name("真君", ({"master", "dangmo", "tianzun"}));
   set("title", "武当「"HIY"统摄真武，剪伐天下"NOR"」");
   set("gender", "男性" );
   set("age", 65);
   set("per", 24);
   set("str", 30);
...
void announce_success (object ob)
{
        ...
        if (ob->query_temp("killme_applied") == "done")
        {
                ...
                command("job "+ WHT "" +ob->query("name")+"("+ob->query("id")+")"+"剪伐天下，降妖除魔，功德无量!");
        switch( random(6) ) {
                case 0:
                        i = random(50);
                        ob->add("potential",i+100);
                        tell_object (ob,"\n除了战斗所得，你还赢得了"+chinese_number(i+100)+"点潜能的奖励！\n");
                        break;
                case 1:
                        i = random(100);
                        ob->add("potential",i+100);
                ...
                case 4:
                        ...
                        i = random(800);
                        ob->improve_skill(name, i+ob->query("combat_exp")/2000);
                        tell_object (ob,"\n除了战斗所得，你还赢得了"+chinese_number(i+ob->query("combat_exp")/2000)+"点"+
                        to_chinese(name)+"技能的奖励！\n");
                        break;
                case 5:
                        i = random(20);
                        silver->set_amount(i+ob->query("combat_exp")/800000);
...
//lpg 19991129 增加连续杀妖次数奖励
        j=((ob->query("max_mana")+ob->query("max_force"))/1000+1);
        if ((ob->query("killme_number")>1) && (ob->query("killme_number") > j))
        {
                ob->delete("killme_number");
                j=random(j)+1;
                command("job "+ WHT "" +ob->query("name")+"("+ob->query("id")+")"+"降妖超过"+chinese_number((ob->query("max_mana")+ob->query("max_force"))/200+1)+"次，望再接再励！");
        switch( random(6) ) {
                case 0:
                        i = random(100);
                        ob->add("potential",(i+100)*j);
                        tell_object (ob,"\n你又赢得了"+chinese_number((i+100)*j)+"点潜能的奖励！\n");
                        break;
                case 1:
                        i = random(300);
                        ob->add("potential",(i+100)*j);
