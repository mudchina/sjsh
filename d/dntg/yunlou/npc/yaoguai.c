	...
	int max_kee, max_sen, ratio, exp_r, pot_r;
	    ...
	    if(environment(owner_ob)==environment() && living(owner_ob)) {
		    ...
		    if(ratio<0) ratio=0;
                    ...
                    exp_r=exp_reward*ratio/85;
                    pot_r=pot_reward*ratio/85;
	    } else {
		message_vision("$N死了。\n",this_object());
		exp_r=0;
		pot_r=0;
        ...
        if (owner_ob->query_temp("mieyao/level")==9)
                {
                        int dx_r=random(7000);
                        owner_ob->add("daoxing",dx_r);
                        tell_object(owner_ob, "你得到了"+COMBAT_D->chinese_daoxing(dx_r)
        +"道行!");
...
message("shout",HIM"【"HIW"斩妖除魔"HIM"】"HIW"李靖(Li jing)："HIM"据说" HIW+ owner_ob->query("name")+HIM"协助天庭斩妖除魔有功,玉皇大帝赐"HIW+ to_chinese(skill)+HIM"一级！\n"NOR,users());
    			}
                    }
