//Room: st0.c 深潭
//Date: Oct. 21 1997 by That
#include <ansi.h>

inherit ROOM;
void create()
{
    set("short","深潭");
    set("long",@LONG
你命大，正好跌在舍身崖下深潭中。昏迷中醒来，四周黑蒙蒙，什么都看
不清。潭水发着恶臭，偶尔还触及一两根不知是人是兽的枯骨，让你全身直起
鸡皮疙瘩。你漂浮在水中，无助至极。情急之下，四处张望(wang)。
LONG);
    set("no_fight",1);
    set("no_sleep_room",1);
    set("objects", ([
        __DIR__"npc/frog" : 1,
    ]) );
    set("no_clean_up", 0);
    setup();
}
void init()
{
    add_action("do_wang", "wang");
}
int do_wang()
{
    object ob;
    ob = this_player();
    message_vision(HIC"$N极目四望，还是发现有出路的，于是求生试探之心大动。\n"NOR, ob);
    switch (random(4))
    {
       case 0 : set("exits/east", __DIR__"st1");break;
       case 1 : set("exits/east", __DIR__"st0");break;
       case 2 : set("exits/east", __DIR__"st0");break;
       default: set("exits/east", __DIR__"st0");
    }
    switch (random(4))
    {
       case 0 : set("exits/west", __DIR__"st0");break;
       case 1 : set("exits/west", __DIR__"st1");break;
       case 2 : set("exits/west", __DIR__"st0");break;
       default: set("exits/west", __DIR__"st0");
    }
    switch (random(4))
    {
       case 0 : set("exits/south", __DIR__"st0");break;
       case 1 : set("exits/south", __DIR__"st0");break;
       case 2 : set("exits/south", __DIR__"st1");break;
       default: set("exits/south", __DIR__"st0");
    }
    switch (random(4))
    {
       case 0 : set("exits/north", __DIR__"st0");break;
       case 1 : set("exits/north", __DIR__"st0");break;
       case 2 : set("exits/north", __DIR__"st0");break;
       default: set("exits/north", __DIR__"st1");
    }
    remove_call_out("close_out");
    call_out("close_out", 3);
    return 1;
}

void close_out()
{
  object ob;
  ob = this_player();
  message_vision(HIY"瘴气飘动，$N眼前一片模糊，出路在哪里？\n"NOR, ob);
    if (query("exits/east"))  delete("exits/east");
    if (query("exits/west"))  delete("exits/west");
    if (query("exits/south")) delete("exits/south");
    if (query("exits/north")) delete("exits/north");
}
