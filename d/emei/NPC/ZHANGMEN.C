...
string zname(object);
int rank_me();
#define DIRNAME "/d/emei/npc/"

#define MYFILE "/d/emei/npc/zhangmen.c"
...
void init()
{
	...
	me->create_identity("/d/emei/npc/baimei", "/d/emei/hcaguangchang");
...
string zname(object ob)
{
	if( (string)ob->query("gender") == "女性" ) 
                  return HIM"峨嵋掌门大师姐"NOR;
        else return HIW"峨嵋掌门大师兄"NOR;
}
int rank_me()
{
	...
	exp=exp+dx;
        if( me->query("峨嵋派") == HIM"峨嵋掌门大师姐"NOR
          ||me->query("title") == HIW"峨嵋掌门大师兄"NOR)
		return notify_fail("你已经是掌门身份，还来要什么职位．\n");
	if( me->query("family/family_name") == "峨嵋派")
	{
                if( exp <= 40000 ){
			me->set("title", HIW"峨嵋派入门小道"NOR);
		} else if(exp <= 100000 ){
			me->set("title", HIW"峨嵋派巡山弟子"NOR);
		} else if(exp < 200000 ){
			me->set("title", HIW"峨嵋派巡山统领"NOR);
		} else if(exp <= 400000 ){
		       if(me->query("gender") == "男性"){	
			me->set("title", HIW"峨嵋派镇山大师"NOR);
		       }
		       else me->set("title",HIW"峨嵋派降妖道长"NOR);
		} else if(exp <= 1000000 ){
		       if(me->query("gender") == "男性"){
			   me->set("title", HIW"峨嵋派护法使者"NOR);
		       }
		       else me->set("title",HIW"峨嵋派护法法王"NOR);
		} else if(exp <= 2000000 ){
			me->set("title", HIW"峨嵋派修果大师"NOR);
		} else if(exp <= 4000000 ){
			me->set("title", HIW"峨嵋派白眉护法"NOR);
		} else {
			me->set("title", HIW"峨嵋派天雷继承者"NOR);
		}
