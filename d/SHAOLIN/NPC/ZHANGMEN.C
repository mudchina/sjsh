...
string zname(object);
int rank_me();
#define DIRNAME "/d/shaolin/npc/"

#define MYFILE "/d/shaolin/npc/zhangmen.c"
...
void create()
{
	...
	setup();
        ...
        carry_object("/d/qujing/jindou/obj/jindan")->wield();
}
...
void init()
{
	...
	me->create_identity("/d/shaolin/npc/xuan-ci", "/d/shaolin/wuchang");
...
string zm_apply()
{
	...
	{
		return "你曾背叛过师门，言无信行不轨，岂能出任掌门弟子一职！\n";
	}
	...
	if( (string)ob->query("gender") == "男性" )
		return "好！道兄有意出任掌门大师兄一职？我们就切磋一下吧！\n";
}
...
{
	CHANNEL_D->do_channel(who,"chat","本派掌门大师兄"+ob->query("name")+"今日走马上任。恭请各位仙长多加捧场！");
...
string zname(object ob)
{
	if( (string)ob->query("gender") == "女性" ) 
		return HIM"护寺大师兄"NOR;
        else return HIC"糊里糊涂大师兄"NOR;
}
int rank_me()
{
	...
	exp=(int)me->query("combat_exp");
        if( me->query("title") == "少林寺护寺大师兄"
          ||me->query("title") == "少林寺糊里糊涂大师兄")
	...
	if( me->query("family/family_name") == "少林寺")
	{
			...
			me->set("title", "少林寺寻山小僧");
		} else if(exp <= 200000 ){
			me->set("title", "少林寺客串徒弟");
		} else if(exp <= 500000 ){
			me->set("title", "少林寺护寺徒弟");
		} else if(exp <= 1000000 ){
			me->set("title", "少林寺护法神僧");
		} else if(exp <= 2000000 ){
			me->set("title", "少林寺俗家下山弟子");
		} else {
			me->set("title", "少林寺独霸一方徒儿");
		}
...
HIR""+ob->query("name")+"对"+me->query("name")+"说：你可以做"+me->query("title")+"。\n"NOR, ({me, me}));
	...
	{
                if( exp <= 20000 ){
			me->set("title", "少林寺伙房橱娘");
		} else if(exp <= 50000 ){
			me->set("title", "少林寺沏茶女童");
		} else if(exp < 100000 ){
			me->set("title", "少林寺内院主管");
		} else if(exp <= 200000 ){
			me->set("title", "少林寺医乐院道姑");
		} else if(exp <= 500000 ){
			me->set("title", "少林寺紫须圣姑");
		} else if(exp <= 1000000 ){
			me->set("title", "少林寺月露仙姑");
		} else if(exp <= 2000000 ){
			me->set("title", "少林寺芙蓉圣母");
		} else {
			me->set("title", "少林寺渌水神姬");
		}
