...
string ask_title()
{
        ...
        me = this_player();
        num = (int)me->query("job/nanhai_kill");
        
        if(me->query("family/family_name")!="南海普陀山") return "这位"+RANK_D->query_respect(me)+"是不是搞错了？";

        if(!me->query_temp("nanhai/kill_monster")) return "你今天是不是忘了做点什么？";
        
        if(!me->query("job/nanhai_kill")) return "你毫无战功，何来封赏？";
                
        if(num >= 1000){
        me->set_temp("nanhai/title", 6);
        command_function("say 这位"+RANK_D->query_respect(me)+"，你立下无数功劳，小王无以为报，愿与你共理此国，封你为王。");
        }
        else if(num >= 600){
        me->set_temp("nanhai/title", 5);
        command_function("say 这位"+RANK_D->query_respect(me)+"战功赫赫，威名远播，是国之栋梁！");
        }
        else if(num >= 300){
        me->set_temp("nanhai/title", 4);
        command_function("say 这位"+RANK_D->query_respect(me)+"灭妖无数，功劳显赫，小王必加重用！");
        }
        else if(num >= 200){
        me->set_temp("nanhai/title", 3);
        command_function("say 这位"+RANK_D->query_respect(me)+"保我疆国，卫我臣民，小王自有封赏！");  
        }
        else if(num >= 100){
        me->set_temp("nanhai/title", 2);
        command_function("say 这位"+RANK_D->query_respect(me)+"常年征战，捷报频传，小王自有答谢！");
        }
        else if(num >= 50){
        me->set_temp("nanhai/title", 1);
...
string ask_need()
{
        ...
        times = (int)me->query("job/nanhai_kill");

        if(me->query("family/family_name")!="南海普陀山") return RANK_D->query_respect(me)+"非我家臣，有否功劳与小王何干？";
...
//    if (base_name(environment(me))!="/d/nanhai/qiandian") return notify_fail("这里不是金銮殿，怎么能接旨？\n");
        
        if(!me->query_temp("nanhai/title")) return notify_fail("你要行君臣大礼吗？\n");
        
        if( me->query_temp("nanhai/title") == 1) title = titles1[random(sizeof(titles1))];
        else if( me->query_temp("nanhai/title") == 2) title = titles2[random(sizeof(titles2))];
        else if( me->query_temp("nanhai/title") == 3) title = titles3[random(sizeof(titles3))];   
        else if( me->query_temp("nanhai/title") == 4) title = titles4[random(sizeof(titles4))];
        else if( me->query_temp("nanhai/title") == 5) title = titles5[random(sizeof(titles5))];
        else if( me->query_temp("nanhai/title") == 6) title = titles6[random(sizeof(titles6))];
        ...
        me->set("title", "南海普陀山蠙城"+title);
        me->delete_temp("nanhai/title");
        me->delete_temp("nanhai/kill_monster");   
        return 1;
}                                       
