...
#define DIRNAME "/d/swordman/npc/"

#define MYFILE "/d/swordman/npc/zhangmen.c"
...
void init()
{
      ...
      me->create_identity("/d/swordman/npc/xun", "/d/swordman/wuguan");
...
void reset_me (object me)
{
        ...
        for(i=0; i<sizeof(inv); i++)
        {
         ...
         me->set_name("大弟子", ({ "yuehua jianshi", "jianshi" }) );
        me->set("gender", "男性" );
          me->set("title","月华剑士");
...
int convert_identity (object me, object ob)
{
        ...
        if (ob->query("gender")=="男性") {
        	me->set_name(ob->query("name"), ({ "yuehua jianshi", "jianshi" }) );
	       	me->set("title","月华剑士");
        } else{
        	me->set_name(ob->query("name"), ({ "yuehua jianshi", "jianshi" }) );
	       	me->set("title","月华剑士");
...
"你曾叛师欺祖，言无信行不轨，岂能出任掌门弟子一职！\n";
        ...
        ob->set_temp("swordman_zm_applied", 1);
        if( (string)ob->query("gender") == "男性" )
                return
"好！师兄有意出任月华剑士一职？我们就切磋一下吧！\n";
        else return "好！师姊有意出任月华剑士一职？我们就切磋一下吧！\n";
}
...
int accept_fight(object ob)
{
        ...
        if (ob->query_temp("swordman_zm_applied"))
        {
...
ob->query_temp("swordman_zm_applied"))
                {
                        message_vision
("$N皱了皱眉道：此次比武我未能静心尽力，希望重新来过。\n", me);
                ...
                if( ob->query_temp("swordman_zm_applied") != 1 )
                        return 1;
...
CHANNEL_D->do_channel(who,"chat",str+ob->query("name")+"今日走马上任。恭请各位仙长多加捧场！");
        ob->delete_temp("swordman_zm_applied");
        me->set("new_player",1);
