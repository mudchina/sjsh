...
void create()
{
        set_name(WHT"小圆子"NOR, ({"xiao yuanzi","yuanzi"}));
...
{
        object m,mm;
        message_vision("$N向小圆子问道：道兄，开饭了吗？\n", this_player());

        if((int)this_player()->query("food") >=(int)this_player()->max_food_capacity()*90/100 )
        return notify_fail("小圆子笑道：没那，等开饭时再来吧！\n");

        if((present("bao zi", this_player())))
        return notify_fail("小圆子笑道：不要拿着手里的，还看着锅里的！￥？\n");
	
	if ( present("bao zi", environment()) )
                return notify_fail("小圆子笑道：那不是有一块呢嘛，吃完再说！\n");
        ...
        message_vision("小圆子对$N笑道：饿了吧，慢点吃。\n", this_player());
return 1;
}
