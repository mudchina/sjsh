...
int do_climb(string arg)
{
  ...
  if (!((string)me->query("family/family_name")=="平民")
    ...
    {
      message_vision("二重天神拦道：此间并非戏耍之处，请立即离开!\n", me);
    }
  else
    {
      if ((int)me->query_skill("dodge", 1)<80 && (int)me->query_skill("moondance", 1) < 100)
        {
	  message_vision("$N小心翼翼的往上爬了一点，觉得头晕眼花，就赶紧爬了下来．\n", me);
      ...
      else
        {
          message_vision("$N轻轻地一腾就上了树藤,一会便消失在无尽的香花之中了。\n", me);
           me->move("/d/calvin/lanling/san.c");
        }
    }
