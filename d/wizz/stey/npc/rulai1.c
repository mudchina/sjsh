...
int do_task()
{
        ...
        if( (tt=(int)me->query("rulaitask/time_start")) &&
		!me->query("rulaitask/done")) {
	    ...
	    // set a flag "rulaitask/level_change" to judge the monster level.
	    
	    
	} else { // job done
	    if(tt<=time() &&
                    time()<tt+360) {
		message_vision("$N对手中照妖镜一看，妖魔已除尽,不妨先去歇息。\n",
			this_object(), me);
		return 1;
	    }
	   
	}
	me->delete("rulaitask/time_start");
