...
void check()
{
     ...
     {        
        ...
        message_vision(HIR"$N吹了起横笛，顿时一只大肥猪向$n直冲而去！\n",ob,me);

        if(random(2))

                {
              message_vision(HIR"$N被大肥猪撞了个人仰马翻！\n"NOR,me);
              me->add("eff_kee",-2*query("max_kee")/8);
              me->add("eff_sen",-2*query("max_sen")/8);
                }
        else
        message_vision(HIR"$N对着大肥猪大喝一声：停！大肥猪乖乖从旁边溜开了！\n"NOR,me);
        }
        break;
...
void die()
{
        ...
        {
        ob->start_busy(3);
message("chat",HIY+"【黄金十二宫】雅典娜(Yadian na):"+ob->name()+"闯过了第一宫白羊宫，继续向黄金战士前进！\n"NOR,users()); 
