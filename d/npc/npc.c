...
//                        carry_object(base_name(inv[i]))->wield();
                  if (inv[i]->query("no_zm"))
                  { if( random(4)==0) carry_object(base_name(inv[i]))->wield();                  }else{carry_object(base_name(inv[i]))->wield();}     
              }
...
//                        carry_object(base_name(inv[i]))->wear();
                if (inv[i]->query("no_zm"))
                { if( random(2)==0) carry_object(base_name(inv[i]))->wear();  
                }else{carry_object(base_name(inv[i]))->wear();}                
}
...
void die()
{
        ...
        ob->add("daoxing",reward*4);
        ob->add("potential", reward*2);
...
//        tell_object(users(),HIW"【奖励】:"HIY+ob->name()+"成功保护了门派安全，受到了门派奖励\n"NOR);
        ...
        MONITOR_D->report_system_object_msg(ob,"得到了"+COMBAT_D->chinese_daoxing(reward)+"道行和"
                ...
                + reward/2 + "点潜能！");
       MONITOR_D->report_rumor_object_msg(ob,"杀死心魔得到了"+COMBAT_D->chinese_daoxing(reward)+"道行和"
