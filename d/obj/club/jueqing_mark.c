...
#include <ansi.h>
#include <weapon.h>
#define CLUB_NAME "绝情谷"
#define CLUB_ID "jueqing"
#define CLUB_CHANNEL "jueqing"
#define CLUB_MARK "jueqing necklace"
inherit SWORD;
...
void create()
{
        set_name(HIR"绝情令"NOR,({"jueqing necklace","necklace","club_mark","sword","jian"}));
	set_weight(0);
...
这是一块火红色的令牌，上面大大的一个“情”字。凡见此令牌者，
如谷主亲临，所有帮众必须听令。
你可以用(help jueqing)来查看有什么绝情谷指令。若想了解绝情谷
的历史可以用(about jueqing)来取得进一步的资料。
...
string show_level(object user)
{
 ...
 mapping info;
 string str;
 if(wizardp(user)) return "【荣誉会员】绝情谷荣誉帮众  ";
 ...
 switch(rank)
 {
 	case 6: str+="绝情谷绝情谷主  "; break;
 	case 5: str+="绝情谷副谷主    "; break;
 	case 4: str+="绝情谷使者      "; break;
 	case 3: str+="绝情谷护法      "; break;
 	case 2: str+=info["info1"]+"分堂主    "; break;
 	default: 
 		if(info["info1"]!="无")
 		{
 		       str+=info["info1"]+"副堂主    "; break;
 		} else str+="绝情谷一般帮众  ";
 }
 return str;
}
...
int do_include(string arg)
{
	...
	me=this_player();
		...
		if(sizeof(ob_info))
		{
			if(me_info["level"] != 2 ) return notify_fail("只有分堂主能将帮众收入分堂。\n");
	...
	me=this_player();
		...
		if(sizeof(ob_info))
		{
			...
			if( (int) member_array(sub, query("sub_club"))<0 )
				return notify_fail("目前可用的分堂为 <天绝堂> <无情堂> <霹雳堂> <神剑堂> \n");
			ob_info["level"]=2;
			ob_info["info1"]=sub;
			CLUB_D->set_member(CLUB_ID,getuid(ob),ob_info);
			if(!mark=present(CLUB_MARK,ob))
...
int do_givepower(string arg)
{
	...
	me=this_player();
		...
		if(sizeof(ob_info))
		{
			...
			if(level > 5 || level < 1 )
				return notify_fail("目前等级分为 <1:帮众> <2:堂主> <3:护法> <4:使者> <5:副谷主> <6:绝情谷主> \n");
			if( level > ob_info["level"] )
			{
				type="提升";
				res ="并和$n握手勉励一番";
...
int do_handover(string arg)
{
	...
	me=this_player();
		...
		if(sizeof(ob_info))
		{
			...
			message( "channel:jueqing",CYN"【"+CLUB_NAME+"】"+me->name(1)+"("+capitalize(me->query("id"))+")将"+CLUB_NAME+"指挥权移交给"+ob->name(1)+"("+capitalize(ob->query("id"))+")。\n"NOR, users() );
			ob->save();
			me->save();
			return 1;
		}
...
int do_leaveclub(string arg)
{
	...
	message( "channel:jueqing",CYN"【"+CLUB_NAME+"】"+me->name(1)+"("+capitalize(me->query("id"))+")主动退出了"+CLUB_NAME+"。\n"NOR, users() );
...
int do_help(string arg)
{
 if(arg=="jueqing")
 {
  write(@HELP
       ...
       │  accept <人名>               │让某人加入帮会    │护法以上    │
       │  kickout <人名>              │将某人逐出帮会    │护法以上    │
       │  subleader <人名> to <分堂名>│升级某人为分堂主  │护法以上    │
       │  retire <人名>               │撤除某人分堂主职位│护法以上    │
       │  givepower <人名> to <等级>  │改变某人的职位    │使者以上    │
       │  handover <人名>             │掌门职务交接      │帮主        │
       │  listmember                  │帮会名册          │使者以上    │
       └───────────────┴─────────┴──────┘
	 ◎ 使用 accept 指令必须对方先用 joinclub 指令申请才能生效。
         ◎ 绝情谷目前有四个分堂: 【天绝堂】、【无情堂】、【霹雳堂】、【神剑堂】。
         ◎ 等级分为: <1:帮众> <2:堂主> <3:护法> <4:使者> <5:副谷主> <6:绝情谷主>。
