...
#include <ansi.h>
#include <weapon.h>
#define CLUB_NAME "无忧宫"
#define CLUB_ID "wuyou"
#define CLUB_CHANNEL "wuyou"
#define CLUB_MARK "wuyou necklace"
inherit SWORD;
...
void create()
{
        set_name(HIM"无忧令"NOR,({"wuyou necklace","necklace","club_mark","sword","jian"}));
	set_weight(0);
...
这是一个五彩玉石令牌，隐隐幻出无忧两字，此乃无优宫之信物。
你可以用(help wuyou)来查看有什么无忧宫指令。若想了解无忧宫
的历史可以用(about wuyou)来取得进一步的资料。
...
LONG
	...
	set("club_id",CLUB_ID);
	set("club",CLUB_NAME);
	set("sub_club",({"情义堂","妖神堂","水帘堂","兰州堂","五洲堂","四海堂","神州堂","中原堂","天地堂","古墓堂","侠客堂","风雨堂","日月堂","星海堂","竞技堂","无敌堂","无情堂","血魔堂","心魔堂",}));
...
string show_level(object user)
{
 ...
 mapping info;
 string str;
 if(wizardp(user)) return "【荣誉会员】无忧宫荣誉帮众  ";
 ...
 switch(rank)
 {
 	case 10: str+="无忧宫宫主      "; break;
 	case 9: str+="无忧宫宫主相公  "; break;
 	case 8: str+="无忧宫少宫主    "; break;
 	case 7: str+="无忧宫宫主护卫  "; break;
 	case 6: str+="无忧宫资深长老  "; break;
 	case 5: str+=info["info1"]+"分堂主    "; break;
 	case 4: str+=info["info1"]+"副堂主    "; break;
 	case 3: str+="无忧宫护法      "; break;
 	case 2: str+="无忧宫宫女      "; break;
...
int do_handover(string arg)
{
	...
	me=this_player();
		...
		if(sizeof(ob_info))
		{
			...
			message( "channel:wuyou",CYN"【"+CLUB_NAME+"】"+me->name(1)+"("+capitalize(me->query("id"))+")将"+CLUB_NAME+"指挥权移交给"+ob->name(1)+"("+capitalize(ob->query("id"))+")。\n"NOR, users() );
			ob->save();
			me->save();
			return 1;
		}
...
int do_leaveclub(string arg)
{
	...
	message( "channel:wuyou",CYN"【"+CLUB_NAME+"】"+me->name(1)+"("+capitalize(me->query("id"))+")主动退出了"+CLUB_NAME+"。\n"NOR, users() );
 ...
 if(arg=="wuyou")
 {
  write(@HELP
	 ...
	 ◎ 使用 accept 指令必须对方先用 joinclub 指令申请才能生效。
         ◎ 无忧宫目前有十八个分堂: 
         【妖神堂】、【情义堂】、【水帘堂】、【兰州堂】、【五洲堂】、【四海堂】
         【神州堂】、【天地堂】、【古墓堂】、【侠客堂】、【风雨堂】、【日月堂】
         【星海堂】、【竞技堂】、【无敌堂】、【无情堂】、【血魔堂】、【心魔堂】
