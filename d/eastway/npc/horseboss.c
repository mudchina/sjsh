...
void create()
{
  ...
  set("gender", "男性");
  set("age", 32);
  set("long","一个身着朴素的老板，是关东大风堂设在长安东郊提供各类坐骑的。\n在这里你可以选择(choose)自己称心如意的坐骑。\n");
...
//              return notify_fail("你的道行太低，不能豢养坐骑。\n");
                return notify_fail("你的道行太低，即使养了坐骑，也会离你而去。\n");
        if( me->query("pet")) 
//              return notify_fail("你已经养了一头坐骑了，请用 dispose 归还你现在的坐骑n");
                return notify_fail("你已经有了一头坐骑了，所谓一山不容二虎。\n请用 dispose 杀了它，再养吧!\n");
        write("您要养哪类坐骑：\n");
    ...
    int  order;
        ...
        sscanf(arg, "%d", order);
          ...
          if (order <= 0 || order > 22)  {
                ...
                write("到大风堂做买卖就得守规矩，选了坐骑就不能后悔了，除非你杀(dispose)了它：\n");
                write("您要养哪类坐骑：\n");
                write(" 1. 鼠   2. 牛   3. 虎   4. 兔  5. 龙  6. 蛇\n");
                write(" 7. 马   8. 羊   9. 猴  10. 鸡 11. 狗 12. 猪\n");
                write("13. 猫  14. 熊  15. 狮  16. 狼 17. 豹 18. 狐\n");
...
int check_legal_name(string name, int max_len)
{
    ...
    while(i--)   {
        if( name[i]<=' ' )   {
                write("对不起，坐骑中文字不能用控制字元。\n");
                return 0;
        }
        if( i%2==0 && !is_chinese(name[i..<0]) )  {
                write("对不起，请您用「中文」给坐骑取名字。\n");
                return 0;
        }
    }
...
void get_desc(string arg, object ob)
{
        ...
        write("\n");
        write("坐骑用量词：(Return for default)");
...
void confirm_dispose(string arg, object ob, object pet_ob)
{
   ...
   if( arg == "y" || arg == "Y" )  {
        ob->delete("pet");
        msg="\n$N哀怨地看了$n一眼，倒在血泊之中，一双无神的眼睛居然还睁着。。。\n";
