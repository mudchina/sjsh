...
string zname(object);
int rank_me();
#define DIRNAME "/d/kunlun/npc/"

#define MYFILE "/d/kunlun/npc/zhangmen.c"
...
void init()
{
	...
	me->create_identity("/d/kunlun/npc/guang", "/d/kunlun/dadian");
...
void master_announce (object me, object who, object ob)
{
	...
	ob->delete_temp("kunlun_zm_applied");
	me->set("new_player",1);
...
int rank_me()
{
	...
	exp=exp+dx;
        if( me->query("昆仑派") == HIM"昆仑掌门大师姐"NOR
          ||me->query("title") == HIW"昆仑掌门大师兄"NOR)
		return notify_fail("你已经是掌门身份，还来要什么职位．\n");
          if( me->query("family/family_name") == "昆仑山玉虚洞")
	{
                if( exp <= 40000 ){
			me->set("title", HIW"昆仑派入门小道"NOR);
		} else if(exp <= 100000 ){
			me->set("title", HIW"昆仑派巡山弟子"NOR);
		} else if(exp < 200000 ){
			me->set("title", HIW"昆仑派巡山统领"NOR);
		} else if(exp <= 400000 ){
		       if(me->query("gender") == "男性"){	
			me->set("title", HIW"昆仑派镇山大师"NOR);
		       }
		       else me->set("title",HIW"昆仑派降妖道长"NOR);
		} else if(exp <= 1000000 ){
		       if(me->query("gender") == "男性"){
			   me->set("title", HIW"昆仑派护法使者"NOR);
		       }
		       else me->set("title",HIW"昆仑派护法法王"NOR);
		} else if(exp <= 2000000 ){
			me->set("title", HIW"昆仑派修果大师"NOR);
		} else if(exp <= 4000000 ){
			me->set("title", HIW"昆仑派白眉护法"NOR);
		} else {
			me->set("title", HIW"昆仑派天雷继承者"NOR);
		}
