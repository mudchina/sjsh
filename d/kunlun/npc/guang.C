...
void create()
{
       set_name("广成子", ({"guang chengzi","guang","zi","chengzi"}));
       set("long","元始天尊大弟子。\n");
        set("title",HIG"元始天尊大弟子"NOR);
       set("gender", "男性");
       set("age", 51);
       set("class", "kunlun");
...
void attempt_apprentice(object ob,object me)
   {
         ...
         command("say 道兄的道行差了些，又怎么能学我的剑法呢？" + RANK_D->query_respect(ob) + "还需多加努力才能学到真功夫。\n"); 
        return; 
        } 
                   if (me->query_skill("yuxu-spells")  < 110 ) { 
         command("say 道兄的玉虚仙法差了些，又怎么能学我的剑法呢？" + RANK_D->query_respect(ob) + "还需多加努力才能学到真功夫。\n"); 
         return; 
         } 
         ...
         command("say 好极，好极，时下妖魔当道，鬼怪横行。象"  + RANK_D->query_respect(ob) +"这样的正义之士却是少见！\n");; 
             ...
             command("recruit " + ob->query("id") ); 
              ob->set("title", HIM"昆仑山玉虚洞仙剑传人"NOR); 
              ob->set("class","kunlun" ); 
       return; 
       } 
   command("shake");
