...
int main(object me, string arg)
{
   ...
   if(!wizardp(me)){
    ...
    if (me->query("age") < 15)
         return notify_fail("你连十五岁都没到，就学坏啊！\n");
    ...
    if (obj->query("age") < 15)
         return notify_fail("你该不是饥渴到极点了吧,连幼女也不放过!?\n");

    if (!obj->is_character()) return notify_fail("哇,你这样做被同行看见了要笑你变态的。\n");
    ...
    {
      message_vision("$N被按到在地，大声呼救：有色狼啊!非礼啊~~~  拼命挣扎。\n",obj,me);
