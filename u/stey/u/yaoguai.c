...
void die()
{
	...
	int max_kee, max_sen, ratio, exp_r, pot_r;
		...
		ratio=0;
            ...
            exp_r=exp_reward*ratio/100; exp_r=exp_r*3/2;
            pot_r=pot_reward*ratio/100; pot_r=pot_r*3/2; //added by vikee 00-12-8 23:37
        ...
        +"道行!");
		    ...
		    str="得到了 "+exp_r+"/"+exp_reward+" 点武学经验和 "+
			pot_r+"/"+pot_reward+" 点潜能通过第 "+
			owner_ob->query_temp("mieyao/level")+" 级妖"+
			".\n";
		if (owner_ob->query_temp("mieyao/level")==9 && ratio>50) {
          skill=give_reward(owner_ob);
         str+= "并且被奖励了一级 "+to_chinese(skill)+".\n";
   		if (skill != "none"){
