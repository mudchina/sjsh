...
//	return "造成 " + damage + " 点" + type + "。\n";
	...
	switch( type ) {
		...
		case "劈伤":
			...
			else if( damage < 50 ) return "结果在$p$l划出一道细长的血痕。\n";
			else if( damage < 120 ) return "结果「嗤」地一声，在$n身上划出一道伤口！\n";
			else if( damage < 220 ) return "结果「嗤」地一声，在$n身上划出一道血淋淋的伤口！\n";
			else if( damage < 320 ) return "结果「嗤」地一声，在$n身上划出一道又长又深的伤口，溅得$N满脸鲜血！\n";
			else return "结果只听见$n一声惨嚎，$w已在$p$l划出一道深及见骨的可怕伤口！！\n";
			break;
		case "枪伤":
		case "刺伤":
			if( damage < 10 ) return "结果只是轻轻地刺破$n的皮肉。\n";
			else if( damage < 50 ) return "结果在$n$l刺出一个创口。\n";
			else if( damage < 120 ) return "结果「噗」地一声刺入了$n$l寸许！\n";
			else if( damage < 220 ) return "结果「噗」地一声刺进$n的$l，使$p不由自主地退了几步！\n";
			else if( damage < 320 ) return "结果「噗嗤」地一声，$w已在$p$l刺出一个血肉模糊的血窟窿！\n";
			else return "结果只听见$n一声惨嚎，$w已在$p的$l对穿而出，鲜血溅得满地！！\n";
			break;
		case "筑伤":
			if( damage < 10 ) return "结果只是轻轻地一触，在$n的皮肤上留下一点白痕。\n";
			else if( damage < 40 ) return "结果在$p的$l留下几道血痕。\n";
			else if( damage < 80 ) return "结果一下子筑中，$n的$l顿时出现几个血孔！\n";
			else if( damage < 160 ) return "结果一下子筑中，$n立刻血流如注！\n";
			else if( damage < 240 ) return "结果「哧」地一声，$n顿时鲜血飞溅！\n";
			else if( damage < 320 ) return "结果这一下「哧」地一声，$n被筑得浑身是血！\n";
		...
		case "瘀伤":
			...
			else if( damage < 100 ) return "结果一击命中，$n闷哼了一声显然吃了不小的亏！\n";
			else if( damage < 150 ) return "结果「砰」地一声，$n退了两步！\n";
			else if( damage < 200 ) return "结果这一下「砰」地一声打得$n连退了好几步，差一点摔倒！\n";
			else if( damage < 280 ) return "结果重重地击中，$n「哇」地一声吐出一口鲜血！\n";
			else if( damage < 380 ) return "结果只听见「砰」地一声巨响，$n像一捆稻草般飞了出去！！\n";
			else return "只听见$n一声惨嚎，整个身体象断了线的风筝一样飞了出去！！\n";
			break;
		case "内伤":
			if( damage < 20 ) return "结果只是把$n打得退了半步，毫发无损。\n";
			else if( damage < 60 ) return "结果$n痛哼一声，在$p的$l造成一处瘀伤。\n";
			else if( damage < 100 ) return "结果一击命中，把$n打得痛得弯下腰去！\n";
			else if( damage < 170 ) return "结果$n闷哼了一声，脸上一阵青一阵白，显然受了点内伤！\n";
			else if( damage < 210 ) return "结果$n脸色一下变得惨白，昏昏沉沉接连退了好几步！\n";
			else if( damage < 280 ) return "结果重重地击中，$n「哇」地一声吐出一口鲜血！\n";
			else if( damage < 380 ) return "结果「轰」地一声，$n全身气血倒流，口中鲜血狂喷而出！\n";
		...
		case "震伤":                
			...
			else if( damage < 210 ) return "结果$n被$N以劲力一震，胸口有如受到一记重锤，连退了五六步！\n";
			else if( damage < 280 ) return "结果$N的劲力一下震得$n连退了好几步，差一点摔倒！\n";
			else if( damage < 380 ) return "结果$n被$N的劲力震得全身气血倒流，口中鲜血狂喷而出！\n";
		...
		case "拉伤":
			...
			else if( damage < 280 ) return "只听$n「啊」地一声吐出一口鲜血，原来$p$l内的骨头已经被$w的劲力抽裂！\n";
			else if( damage < 380 ) return "结果随着「咔」地一声，$w已将$n$l处的骨头硬生生地抽断，断骨从肉里冒了出来！\n";
		...
		case "撞伤":
			...
			else if( damage < 30 ) return "结果在$p的$l砸出一个小臌包。\n";
			else if( damage < 60 ) return "结果砸个正着，$n的$l登时肿了一块老高！\n";
			else if( damage < 100 ) return "结果砸个正着，$n闷哼一声显然吃了不小的亏！\n";
			else if( damage < 180 ) return "结果「砰」地一声，$n疼得连腰都弯了！\n";
			else if( damage < 240 ) return "结果这一下「轰」地一声砸得$n眼冒金星，差一点摔倒！\n";
			else if( damage < 320 ) return "结果重重地砸中，$n眼前一黑，「哇」地一声吐出一口鲜血！\n";
		...
		case "抓伤":
			...
			else if( damage < 300 ) return "结果只听见$n一声惨嚎，$N的手爪已在$p的$l处刺出了五个血肉模糊的窟窿！\n";
			else if( damage < 380 ) return "结果「啊」地一声惨叫，$n身上$l处被$N抓下了一大片皮肉，鲜血横飞满地！！\n";
			...
			else if( damage < 280 ) return "结果$n被一下摔倒在地，「噗地」一声喷出了一大口鲜血！\n";
                        else if( damage < 380 ) return "结果被$N这一下摔在地上，$l处的骨头「卡嚓」一声断了！\n";
			return "结果只听见$n一声惨嚎，被$N摔得血肉横飞，惨不忍睹！！\n";
			break;
		case "鞭伤":
		case "抽伤":
			if( damage < 10 ) return "结果只是在$n的皮肉上碰了碰，跟蚊子叮差不多。\n";
			else if( damage < 50 ) return "结果在$n$l抽出一道轻微的紫痕。\n";
			else if( damage < 120 ) return "结果「啪」地一声在$n$l抽出一道长长的血痕！\n";
			else if( damage < 220 ) return "结果只听「啪」地一声，$n的$l被抽得皮开肉绽，痛得$p咬牙切齿！\n";
			else if( damage < 320 ) return "结果「啪」地一声爆响！这一下好厉害，只抽得$n皮开肉绽，血花飞溅！\n";
		...
		default:
			...
			else if( damage < 50 ) str = "结果造成一处";
			else if( damage < 80 ) str = "结果造成一处严重";
			else if( damage < 100 ) str = "结果造成颇为严重的";
			else if( damage < 150 ) str = "结果造成相当严重的";
			else if( damage < 230 ) str = "结果造成十分严重的";
			else if( damage < 320 ) str = "结果造成极其严重的";
...
varargs int do_attack(object me, object victim, object weapon, int attack_type)
{
	...
	&& random(8)==0 && victim->query("combat_exp") > random(me->query("combat_exp"))) {
	   message_vision(HIG+ "$n闪过$N之攻势，仰天长啸，身形展动，使出了[云程九万里]的心法！！\n"+NOR, me, victim);
           do_attack(victim, me, weapon, TYPE_REGULAR);
           do_attack(victim, me, weapon, TYPE_REGULAR);
           victim->set("sp/yuncheng_use", time());
           } // xueshan sp 云程九万里 xintai 3/24/2001
        }
        ...
        if( random(ap + pp)< mod_val ) {
            ...
            if ( victim->query("sp/putishu") && victim->query_skill("parry")> 200
          && time()-(int)victim->query("sp/putishu_use") > 15   
          && victim->query_skill_mapped("parry")=="qianjun-bang"
	  && random(8)==0 && victim->query("combat_exp") > random(me->query("combat_exp") )) {
	   message_vision(HIG + "$n深吸一口气，全身真气流转，方寸不传之密[菩提树]信手使出！！\n"+NOR, me, victim);
           do_attack(victim, me, weapon, TYPE_REGULAR);
           do_attack(victim, me, weapon, TYPE_REGULAR);
           victim->set("sp/putishu_use", time());
           } // fc sp 菩提树 xintai 3/24/2001 
            ...
            if (damage > 50 && victim->query("sp/xiuluo") && victim->query_skill("force") > 200
            ...
            && random(10)==0 && victim->query("combat_exp") > random(me->query("combat_exp"))) {
             message_vision(HIG + "$n一声怒吼箭步上前，使出了[修罗霸王靠华山]！！！\n"+NOR, me, victim);
             message_vision(HIC + "修~~~罗！\n"+NOR, me, victim);
             do_attack(victim, me, weapon, TYPE_REGULAR);
             message_vision(HIC + "      霸~~~王！！\n"+NOR, me, victim);
             do_attack(victim, me, weapon, TYPE_REGULAR);
             message_vision(HIC + "       	靠华山！！！\n"+NOR, me, victim);
             do_attack(victim, me, weapon, TYPE_REGULAR);
             victim->set("sp/xiuluo_use", time());// longgong sp 修罗霸王靠华山 xintai 3/24/2001
            }
            ...
            if( damage_bonus > 0 ) {
               ...
               if (victim->query("sp/huijian") && victim->query_skill("moonforce", 1) > 150
               && time() - (int)victim->query("sp/huijian_use") > 30
               && victim->is_busy() && victim->query_skill_mapped("force") == "moonforce"
               && random(8)==0 && victim->query("combat_exp") > random(me->query("combat_exp")) ){
               message_vision(HIG + "$n运起月宫密传之[兰心慧剑断情丝]心法，借$N之力恢复了行动！！！\n"+NOR, me, victim);
               victim->start_busy(1, "你正在运用[兰心慧剑断情丝]心法！！\n");
               victim->set("sp/huijian_use", time());
               } // moon sp 兰心慧剑断情丝 xintai 3/24/2001
              } 	 
	    ...
	    && random(5)==0 && victim->query("combat_exp") > random(me->query("combat_exp")) ) {
	    message_vision(HIG + "$n使出[魑魅魍魉]身法，犹如鬼魅，将$N的劲力卸开部分！！！\n"+NOR, me, victim);		
	    damage -= victim->query_skill("ghost-steps");
	    victim->set("sp/chimeiwangliang_use", time());
	    } // hell sp 魑魅魍魉 xintai 3/24/2001
	    ...
	    if (victim->query("sp/xinghuoliaoyuan") && victim->query_skill("force") > 200
            ...
            && random(7)==0 && victim->query("combat_exp") > random(me->query("combat_exp"))
            && victim->query("kee")*100/victim->query("max_kee") <= 15 ) {
            message_vision(HIG + "正在危急之际，忽然间$n只觉一股热流自脚底涌泉穴升起，好一个[星火燎原]神功！！！\n"+NOR, me, victim);	
            victim->receive_curing("kee", 10 + damage );
            victim->set("sp/xinghuoliaoyuan_use", time());
            }// jjf sp 星火燎原 xintai 3/24/2001	
            ...
            && random(3)==0 && random(victim->query("force")) > 500) {
            message_vision(HIG + "$n深吸一口气，在最后关头硬是将要害避过,[万流归宗]确是非同凡响！！\n"+NOR, me, victim);
            wanliu = 0;
            victim->set("sp/wanliu_use", time());
             } // shushan sp 万流归宗 xintai 3/24/2001
    ...
    if ( (damage != RESULT_DODGE) && (damage != RESULT_PARRY) )
		{message_vision(result, me, victim );
		 if ( damage == RESULT_FORCE && me->query("eff_kee")<0 )
		      message("channel:rumor",HIM"【空穴来风】某人："+me->name()+"中了"+victim->name()+"的拨转招式之法，自己把自己打死了。\n"NOR,users());
...
if(killer->query("job/sxy") >= 1 && victim->query("sxy") >= 1
	...
	{
	   tell_object(killer,"你杀死了一个妖怪，你的功绩提高了！\n");
