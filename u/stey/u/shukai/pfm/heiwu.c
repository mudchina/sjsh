...
int cast(object me, object target)
{
   ...
   if( random(ap + dp) > dp ) 
   {
     ...
     if( damage > 0 ) 
     {
        ...
        target->receive_wound("sen", damage/2, me); //原来是 target->receive_wound("sen", damage*2, me);
        target->receive_damage("kee", damage, me); //原来是target->receive_damage("kee", damage*2, me);
        target->receive_wound("kee", damage/2, me); //原来是target->receive_wound("kee", damage*3, me);
//        target->start_busy(1);
        ...
        me->receive_wound("sen", -damage/2, target);//原来是        me->receive_wound("sen", -damage*2, target);
        me->receive_damage("kee", -damage, target);
        me->receive_wound("kee", -damage/3, target);//原来是        me->receive_wound("kee", -damage*3, target);
