...
message("system",HIG"【太平盛世】袁天罡(yuan tiangang)："+me->name()+"找到了"+quest_item->name()+",交给"+who->name()+"得到了"+daoxing(exp)+"道行和"+chinese_number(pot)+"点潜能。\n"NOR,users());
    score=me->query_temp("dynamic_find");        
                        if(score+1==10){
            me->delete_temp("dynamic_find");
            me->add("potential",2000);
            tell_object(me,sprintf("你真行，你得到了"HIY"%s"NOR"点潜能的附加奖励！\n"NOR, daoxing(exp),chinese_number(pot)));        
        }
...
string dyn_quest_list()
{
   ...
   output="";
        ...
        if(already_spreaded(quests[i*2]))
            ...
            if(random(2))output += sprintf(HIG"%10s"NOR"的"HIY"%10s"NOR"（%s） "HIW"【 "HIY" 已完成"NOR" 】\n",item->query("owner_name"),item->query("name"),item->query("id"));
            else output=sprintf(HIG"%10s"NOR"的"HIY"%10s"NOR"（%s）"HIW"【 "HIY" 已完成"NOR" 】\n",item->query("owner_name"),item->query("name"),item->query("id"))+output;
        }
        output =HIR"※仙侣奇缘使命榜※"NOR+
                "━━━━━━━━━━━━━━━━━━━\n"+output+
                "━━━━━━━━━━━━━━━━━━━\n\n";
