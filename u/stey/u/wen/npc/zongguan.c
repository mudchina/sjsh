...
void create()
{
	set_name(""HIY"文"NOR"", ({"zongguan wen", "wen"}));
	set("long", "他是发送装备的总管 文,你可以(list)查看想要的东西。\n");
	set("gender", "女性");
	set("age", 18);
	set("per", 40);
	set("title",""HIG"装备总管");
	...
	set("chat_chance",20);
    set("chat_msg", ({
        "文轻声告诉你：你可以用list指令列出想要的东西！\n",
        "文轻声告诉你：你可以用yao指令向总管我索取装备。\n",
        "文很神气的说：我就是总管，总管就是我！\n",		
        }));
	setup();
...
int do_fight(string arg)
{
  ...
  {
	message_vision("$N看样子想打架，但文很有涵养,不和$N动粗。\n",who,me);
     return 1;
  }
  return 0;
}
...
int do_kill(string arg)
{
  ...
  if (arg && present(arg,environment(who))==me)
  {
message_vision("$N看样子想杀死文，但文是文化人,没跟$N一般见识。\n",who,me);
...
int do_cast(string arg)
{
  ...
  message_vision("$N对着文结结巴巴的念了几句咒语，自己都感觉难听死了。\n",who,me);
...
int do_ji(string arg)
{
  ...
  message_vision("$N想对文祭法宝，前思后想，还是没敢。\n",who,me);
...
int do_throw(string arg)
{
  ...
  message_vision("$N掏出暗器，看了看文，信心不足，又把暗器收了起来。\n",who,me);
...
int do_list(string arg)
{
  ...
  message("vision",""HIG"\n─────────────────────────────────────
          \n"HIY"盔甲："HIC"
          1.金甲(armor)     2.披风(pifeng)    3.盾牌(shield)     4.铁盔(kui)    
          5.仙衣(cloth)     6.踏云靴(shoes)   7.法冠(bonnet) 8.养身丹(dan)
          \n"HIY"兵器："HIC" 
          1.降魔棒(stick)   2.青虹剑(sword)   3.软龙筋(whip)     4.金刀(blade)
          5.短铁爪(zhua)    6.黄铜禅杖(staff) 7.宣花斧(axe)      8.月牙铲(fork)
          9.梅花锤(hammer)  10.点金枪(spear)  11.凤凰琴(qin)
"HIG"\n─────────────────────────────────────\n"NOR"",who,me);
		...
		sscanf(arg,"%s",type);
			...
			case "armor":
				...
				ob->set("no_sell", 1);
        			...
        			ob->set("no_give", 1);
				ob->move(who);
				tell_object(who,"文把"+ob->query("name")+"交给你,并告诉你:带好，别乱丢哦！\n");
				break;
			case "pifeng":
				...
				break;
			case "dan":
				ob=new("/d/qujing/tianzhu/obj/dan.c");
				ob->set("no_sell", 1);
			        ob->set("no_drop", 1);
        			ob->set("no_give", 1);
				ob->move(who);
