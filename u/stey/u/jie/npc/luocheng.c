...
{       
   ...
   if((string)me->query("family/family_name")=="将军府") {
     message_vision("罗成道：你要学我罗家不传秘技「梅花六出」？\n", me);
     if( me->query("pfm_meihua_on") == 1 ) return ("不是已经告诉你了吗？\n");
     if( (int) me->query("combat_exp") < 40000 ) return ("凭你现在的修为，恐怕要坠了我罗家的一世英名。\n");
     if( (int) me->query("max_force") < 800 ) return ("你现在内力太低，施展「梅花六出」恐危及性命。\n");
     message_vision("罗成道：为了保家卫国，你附耳过来！\n", me);
     me->set("pfm_meihua_on", 1);
     return ("「梅花六出」的秘诀已经告诉你了，去罢！\n");
   }
   command("kick");
   return ("想刺探我家传秘技，好大的胆子！\n");
}
