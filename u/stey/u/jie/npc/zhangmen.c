...
int convert_identity (object me, object ob)
{
	...
	ob->set("zhangmen/title","普陀山弘法大师");
	ob->set("title","普陀山弘法大师");
	ob->set("family/zm", 1);
	ob->save();
...
CHANNEL_D->do_channel(who,"chat","普陀弘法大师"+ob->query("name")+"今日走马上任。恭请各位仙长多加捧场！");
...
int rank_me()
{
	...
	{
                if( exp <= 20000 ){
			me->set("title", "南海普陀山火工");
		} else if(exp <= 50000 ){
			me->set("title", "南海普陀山入室弟子");
		} else if(exp < 100000 ){
			me->set("title", "南海普陀山知客僧");
		} else if(exp <= 200000 ){
			me->set("title", "南海普陀山护寺弟子");
		} else if(exp <= 500000 ){
			me->set("title", "南海普陀山护法僧");
		} else if(exp <= 1000000 ){
			me->set("title", "南海普陀山护法罗汉");
		} else if(exp <= 2000000 ){
			me->set("title", "南海普陀山首座");
		} else {
			me->set("title", "南海普陀山主持");
		}
