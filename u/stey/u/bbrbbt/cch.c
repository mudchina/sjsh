...
void create()
{
        set_name("中国象棋盘", ({ "cchess board","ccb" }) );
        set_weight(3000000);
...
return notify_fail("这个回合不该你走棋！\n");
        ...
        if( !arg ) return notify_fail("移动棋子：play 横坐标 纵坐标 to 横坐标 纵坐标 \n");

        if( sscanf(arg,"%d %d to %d %d",sCOL,sROW,tCOL,tROW) != 4 )
                return notify_fail("移动棋子：play 横坐标 纵坐标 to 横坐标 纵坐标 \n");
