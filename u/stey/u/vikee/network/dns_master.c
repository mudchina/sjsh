...
// this is the list of muds in LISTNODES.
// we only communicate with these muds.
...
//   if (name == Mud_name()) return;
   ...
   if(undefinedp(junk["PORTUDP"]) ||
      ...
      !check_mud(junk["HOSTADDRESS"], junk["PORTUDP"],
                 junk["NAME"])) {
      /*
     log_file("dns_master",ctime(time())+
     ...
     " "+
     (undefinedp(junk["PORTUDP"])?"unknown_port":junk["PORTUDP"])+"\n");
     */
     return;
        }
