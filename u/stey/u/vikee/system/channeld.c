...
mapping channels = ([
   ...
   "ymdh": ([      "msg_speak": HIC "【妖魔大会】%s: %s\n" NOR,
                   "msg_emote": HIC "【妖魔大会】%s\n" NOR,
           ]),

   
   "chat": ([      "msg_speak": HIC "【仙侣情缘】%s：%s\n" NOR,
          	   "msg_emote": HIC "【仙侣情缘】%s\n" NOR,
           ]),
   //added by vikee           
   "notice":   ([   "msg_speak": NOR "【"HIR"通告"NOR"】"HIY"%s：%s\n" NOR,
        	    "msg_emote": NOR "【"HIR"通告"NOR"】"HIY"%s\n" NOR,
   	       "notice_only" : 1,
         ]),
   "rumor":([   "msg_speak": HIM "【空穴来风】%s：%s\n" NOR,
          	"msg_emote": HIM "【空穴来风】%s\n" NOR,
          "anonymous": "听说"
        ]),
     //this segment code is gettd from dhxy's program
     "jh":   ([     "msg_speak": HIG "【江湖同盟】"+NOR+HIG+"%s：%s\n" NOR,
                    "msg_emote": HIG "【江湖同盟】"+NOR+HIG+"%s" NOR,
                    "msg_color": NOR+HIG,
                    "intermud": GCHANNEL,
                    "intermud_emote": 1,
                    "channel": "jh",
    ...
    "party":([      "msg_speak": WHT "【门派】%s：%s\n" NOR,
                     "msg_emote": WHT "【门派】%s" NOR,
                        "party_only": 1
                ]),
    "lover"	:([ "msg_speak": MAG "【心心相吸】%s：%s\n" NOR,
				"msg_emote": MAG "【心心相吸】%s\n" NOR,
				"lover_only":1
		]),
]);
string party;
...
varargs int do_channel(object me, string verb, string arg, int emote,int allow_chat)
{
   ...
   if( !mapp(channels) || undefinedp(channels[verb]) )
	...
	if ((int)me->query("chblk_chat") && verb == "chat")
		return notify_fail("对不起！你的仙侣情缘频道被关闭了！\n");
   ...
   if( channels[verb]["anonymous"] ) {
		...
		if (userp(me)) {
		    ...
		    sprintf("流言蜚语：%s(%s)。", me->name(),me->query("id")));
		}
	}
