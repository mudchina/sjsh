...
void do_broadcast(string str, object *usr)
{
    int i, j, k;
    object *watcher, obj, *list, who, env;
    string id;

    // check watch_list.
    watcher=filter_array(all_inventory(this_object()), (:userp:));
    if(!(i=sizeof(watcher))) return;
    
    if(!sizeof(watch_list)) return;
	...
	if(!obj) continue;
	id=obj->query("id");
	if(undefinedp(watch_list[id])) continue;
	list=watch_list[id];
	k=sizeof(list);
	if(!k) continue;
	while(k--) {
	...
	while(member_array(0,list)!=-1)
	    list-=({0});
	watch_list[id]=list;
	
	message("broadcast",str,list);
  暩醎�