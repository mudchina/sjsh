...
//	if( random(100)>dp*100/(ap+dp) ) success = 1;
	...
	&& random(5)==0 && target->query("daoxing") > random(attacker->query("daoxing") )) {
	message_vision(CYN + "$n十指连弹，[哧哧]声中，全身被一层氤氲白气笼罩！！！！\n"+NOR, attacker, target);
	success = 0;
...
//      damage=(a_fali-d_fali)/2;
	...
	&& random(6) == 0 && attacker->query("daoxing") > random(target->query("daoxing")) ) {
	message_vision(CYN + "$N面泛红光，伸手往自家头顶一拍，轰然巨响之中，使出了[无名真火]的大神通！！！\n"+NOR, attacker, target);
...
//          damage += (damage*(int)attacker->query_skill("spells"))/300;
            ...
            && random(5)==0 && attacker->query("daoxing") >random(target->query("daoxing") )) {
            message_vision(CYN + "$N面带冷笑，双手结印，竟然是妖魔道中的奥义[魔高一丈]！！！\n"+NOR, attacker, target);
...
//          damage += (damage*(int)target->query_skill("spells"))/300;
            ...
            if(damage_bonus > 3) damage_bonus = 3;
            damage -= damage*damage_bonus;// 原来居然是 +,呵呵，bug吧:) xintai/3/24/2001
            ...
            && random(5)==0 && attacker->query("daoxing") >random(target->query("daoxing") )) {
            message_vision(CYN + "$N脚踏倒八卦，集中精力，运起了镇元门中之[大罗真身]无上神通！！！！\n"+NOR, attacker, target);
            damage += attacker->query_skill("spells")*2;
	...
	int damage, string damage_type, string msg_hit)
{
	...
	&& random(5)==0 && victim->query("daoxing") > random(winner->query("daoxing") )) {
	 message_vision(CYN + "$n口喧佛号双手合十，全身散发出阵阵祥光，佛门心法[明镜台]果然神妙无方！！！！\n"+NOR, winner, victim);
	 damage_qi -= victim->query_skill("spells")*2;
	 if (damage_qi < 1) damage_qi = 1;
	 damage_shen -= victim->query_skill("spells")*2;
	 if (damage_shen < 1) damage_shen = 1;
