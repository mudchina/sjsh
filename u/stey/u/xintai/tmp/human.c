...
void setup_human(object ob)
{
	...
	if( undefinedp(my["kar"]) ) my["kar"] = random(21) + 10;
 		...
 		if (ob->query("combat_exp") <= 2314912)
		qi= (my["con"]+my["str"])/2;
		else qi = (my["con"]+my["str"])/3*2;
		if (ob->query("daoxing") <= 3455999)
		shen= (my["spi"]+my["int"])/2;
		else shen= (my["spi"]+my["int"])/3*2;// xintai 2/25/2001
	...
	if( userp(ob) || undefinedp(my["max_kee"]) ) {
		...
		if(my["max_force"] >= 3000 ) my["max_kee"] += my["max_force"] / 2.5;// xintai 2/25/2001
		else if( my["max_force"] > 0 ) my["max_kee"] += my["max_force"] / 4;
	...
	if( userp(ob) || undefinedp(my["max_sen"]) ) {
		...
		if( my["max_mana"] >= 3000 ) my["max_sen"] += my["max_mana"]/2.5;// xintai 2/25/2001
		else if( my["max_mana"] > 0 ) my["max_sen"] += my["max_mana"]/4;
