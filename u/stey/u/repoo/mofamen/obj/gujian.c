...
void create()
{
        set_name(HIW"骨龙"+HIM"之剑"NOR, ({ "gulong jian", "jian", "sword" }) );
        set_weight(4000);
        if( clonep() )
...
int ji_ob(object victim)
{
        ...
        fabao->add("spi", 1000);
        if( me->is_busy() )
                return notify_fail("你正忙着呢，无法祭出"+HIW"骨龙"+HIM"之剑。\n"NOR);
        if( !victim )
                return notify_fail("你想祭的人不在这里。\n");
        if( where->query("no_magic") )
                return notify_fail("这里不能祭"+HIW"骨龙"+HIM"之剑。\n"NOR);
        if( !me->is_fighting() )
                return notify_fail("只有战斗中才能祭出。\n");
        if( (int)me->query("mana") < 800 )
                return notify_fail("你的法力不能控制"+HIW"骨龙"+HIM"之剑。\n"NOR);

message_vision(YEL"\n$N大喝一声“去!”，只见$n"+YEL"「呼」地一下化做一条骨龙飞到半空。\n"NOR, me, fabao);
