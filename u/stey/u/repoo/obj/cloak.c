...
void init()
{
  ...
  me = this_player();
  if (me)
       set_name (me->name(1)+ "的"+HBRED BLINK HIG"魔法师披风"NOR, ({ "wiz cloak", "cloak" }));
  else
...
me->set("kee", max);
  ...
  me->set("water",max);
  me->clear_condition();
 message_vision( HIB"$N喃喃念起不死族的回复咒文： 撒拉 伊克 庵修姆  \n"NOR+HIR"     "+me->query("name")+" "NOR+HIB"的全部精力迅速恢复了。\n"NOR,this_player());
  return 1;
}
int wave (string arg)
{
...
int do_wakeup (string str)
{
  ...
  if (!(who=present (lower_case(str), environment(this_player()))) )
    ...
    message_vision( HIR"$N将神秘的披风迎风一展，只见一道红光拂过$n的脸庞。\n"NOR+
    HIY"$n渐渐恢复了知觉。\n"NOR,me,who);
...
int do_hunmi(string arg)
{
	object ob;
    object me = this_player();
	if (arg == "repoo" ) return notify_fail("你不可以对repoo这样哦!\n");
