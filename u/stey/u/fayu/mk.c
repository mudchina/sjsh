...
void create() 
{ 
        set_name(HIR"红叶"+HIW"工具箱"NOR + RMK_VERSION, ({ "mk", "maker" }) ); 
        set_weight(100); 
        set("unit", "个" ); 
        set("long", @LONG 
这是一个用来「快速」制造物件的工具，帮助时间不多或有严重  lag 的巫师 
利用线上输入制造简单的物件，不过如果你想提高自己制作的物件品质，最好 
...
saveobj 
    ...
    将你身上的临时物件存档为正式物件。（有一点请注意，存档以后，请update
你刚才制作出来的物件，否则它会保持最初的形态。） 
