...
void create()
{
        set_name(HIR"镇宫宝物"NOR, ({ "zhenggong baowu","baowu"}));
        set_weight(10);
        if( clonep() )
...
int do_jihui(string arg)
{
		...
		me=this_player();
        ob=this_object();
		if(!arg||arg!="baowu") return notify_fail("你要祭回什么？\n");
        ...
        return notify_fail("你身上没有镇宫宝物。\n"); 
        //check ok
		message_vision("$N嘀咕了几句咒语附在镇宫宝物。\n"+HIR"只听「轰」的一声，镇宫宝物在一阵风中中慢慢消失 。\n"NOR, me);
