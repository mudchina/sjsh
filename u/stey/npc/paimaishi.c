...
int do_paimai(string arg)
{
  ...
  if (ob->query("has_transformed"))    return notify_fail("变形的东西不能拍卖了！\n");
...
int begin_paimai(string name,int money,int num,int m)
{
        ...
        set("on_paimai",1);
        command("news " +name+" 底价 "+chinese_number(money)+"两银子 第"+chinese_number(m)+"次\n"NOR);
        if (!(m==3))
...
int finish_paimai(int num,int sign)
{
        ...
        {
                if(owner)
                        tell_object(owner,HIR"你的东西没有卖出去，请在十分钟内来拍卖行取回自己的物品。\n"NOR);
  owner->set("no_sold",1);
        }
        else            
        {
                command("news " +"成交!"+buyer->query("id")+"获得了 "+NOR+query("npaimai/" + sprintf("%d", num))+HIW+" 的所有权。\n"NOR);
                if (buyer)
                        tell_object(buyer,HIR"你获得了 "+NOR+query("npaimai/" + sprintf("%d", num))+HIR+" 的所有权，请在十分钟内来拍卖行取回你买的东西。\n"NOR);
                if (owner)
...
int report_paimai(object buyer,int money,int num)
{
        ...
        set("value",money);
        command("news " +buyer->query("id")+" 出价 "+chinese_number(money)+"两银子 购入"+NOR+query("npaimai/" + sprintf("%d", num))+HIW"的所有权。\n"NOR);
...
{
        m+=1;
        command("news " +id+" 出价 "+chinese_number(money)+"两银子 购入 "+NOR+name+HIW+" 的所有权，第"+chinese_number(m)+"次\n"NOR);
        if (!(m==3))
