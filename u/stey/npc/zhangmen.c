...
#define DIRNAME "/d/sanjie/npc/"

#define MYFILE "/d/sanjie/npc/zhangmen.c"

void create()
{
	set_name("三界魔神", ({ "mo shen", "zhangmen" }) );
	set("gender", "男性" );
	set("age", 30);
	set("str", 30);
	set("per", 30);
...
void init()
{
	...
	me->create_identity("/d/sanjie/npc/zhaoyang", "/d/sanjie/tianyidian4");
...
void reset_me (object me)
{
	...
	me->set_name("三界魔神", ({ "mo shen", "zhangmen" }) );
	me->set("gender", "男性" );
...
string zm_apply()
{
	...
	ob->set_temp("sanjie_zm_applied", 1);
...
int accept_fight(object ob)
{
	...
	if (ob->query_temp("sanjie_zm_applied"))
	{
...
int check_result(object me, object ob)
{
	...
	if (((int)me->query("kee")*100/(1+my_max_kee)) <= 50 )
	{
		...
		if (me->query_temp("zhangmen/kill") && ob->query_temp("sanjie_zm_applied"))
		{
			message_vision ("$N皱了皱眉道：此次比武我未能静心尽力，希望重新来过。\n", me);
			return 1;
		}

		if( ob->query_temp("sanjie_zm_applied") != 1 )
			return 1;	
...
void master_announce (object me, object who, object ob)
{
	...
	ob->delete_temp("sanjie_zm_applied");
	me->set("new_player",1);
