...
mixed main(object me, string arg, int remote)
{
   ...
   if(remote) {
#ifdef INTERMUD_NAME
       str = "\n"HIR"〖  "HIW" "+ INTERMUD_NAME +""HIR" 〗"NOR"\n";
#else
       str = "\n"HIR"〖  "HIW" "+ MUD_NAME +""HIR" 〗"NOR"\n";
#endif
   } else
       str = "\n"HIR"〖  "HIW" "+ MUD_NAME +""HIR" 〗"NOR"\n";
   ...
   if( opt_long || opt_wiz ) {
     ...
     while( j-- ) {
        ...
        str = sprintf("%s%-15s%s",
          str,
        list[j]->name()+"("+capitalize(list[j]->query("id")) + ")",  
   ...
   //small packets maybe arrived to remote machine out of order.
     str += ""HIC"≡"HIY"───────────────────────────────────"HIC"≡"NOR"\n";
     str = sprintf("%s共有 %d 位使用者连线中，系统负担：%s", str, ppl_cnt,
     query_load_average() + "\n");
        }
