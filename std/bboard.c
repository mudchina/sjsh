...
string short()
{
   ...
   if( !pointerp(notes) || !sizeof(notes) )
     return ::short() + " [ √ª”–»Œ∫Œ¡Ù—‘  ±æ∞Â∞Â÷˜£∫"+HIY+query_master()+NOR+"]";
   ...
   if( unread )
     return sprintf("%s [ [32m%d[m ’≈¡Ù—‘£¨[1;36m%d[m ’≈Œ¥∂¡  ±æ∞Â∞Â÷˜£∫"+HIY+query_master()+NOR+"]", ::short(), sizeof(notes), unread);
   else
     return sprintf("%s [ [32m%d[m ’≈¡Ù—‘  ±æ∞Â∞Â÷˜£∫"+HIY+query_master()+NOR+"]", ::short(), sizeof(notes));
}
...
notes[i]["title"]);
msg+="ª∂”≠π‚¡Ÿ»˝ΩÁ…Òª∞                             ±æ’æµÿ÷∑£∫210.34.120.6 6666\n";
...
int do_post(string arg)
{
   ...
   if(!wizardp(me) && environment(me)->query("post_limit"))
      return notify_fail("÷ª”–Œ◊ ¶≤≈ø…“‘‘⁄’‚¿Ô¡Ù—‘°£\n");

   if(!arg) return notify_fail("¡Ù—‘«Î÷∏∂®“ª∏ˆ±ÍÃ‚°£\n");
   if(strlen(arg) > 28 ) return notify_fail("±ÍÃ‚Ã´≥§£°\n");
...
int do_re(string arg)
{
   ...
   if(!arg) return notify_fail("ƒ„“™Reƒƒ∆™Œƒ’¬£ø\n");
   if(!intp(atoi(arg)) ) return notify_fail("ƒ„÷ªƒ‹ReæﬂÃÂµƒƒƒ“ª∆™Œƒ’¬£¨”√re  ˝◊÷±Ì æ\n");
   if(atoi(arg) > sizeof(notes) ) return notify_fail("√ª”–’‚∆™Œƒ’¬£°\n");
...
int do_appoint(string arg)
{
   ...
   if( (string)SECURITY_D->get_status(this_player(1)) != "(admin)"
    ...
    return notify_fail("ƒ„√ª”–»Œ√‚∞Ê÷˜µƒ»®¿˚!\n");
    
    if( !arg)
        return notify_fail("ƒ„“™»Œ√‚À≠Œ™±æ∞Ê∞Ê÷˜£ø\n");

    if( !present(arg,environment(me) ) )
        return notify_fail("ƒ„“™»Œ√¸µƒÕÊº“≤ª‘⁄’‚∏ˆµÿ∑Ω£°\n");

    if(this_object()->query("master") )
                return notify_fail("’‚∏ˆ∞Ê“—æ≠”–∞Ê÷˜¡À£°\n");

    this_object()->set("master",arg);
    message("system",HIR"°æ»Œ√¸π´∏Ê°ø£∫»Œ√¸ "+arg+" Œ™"+this_object()->name()+"∞Â÷˜£°\n"NOR,users());
    save();      
  return 1;
}
...
int do_demit()
{
   ...
   if( (string)SECURITY_D->get_status(this_player(1)) != "(admin)"
    ...
    if(!this_object()->query("master") )
                return notify_fail("’‚∏ˆ∞Ê±æ¿¥æÕ√ª”–∞Ê÷˜£°\n");
    message("system",HIR"°æ»Œ√¸π´∏Ê°ø£∫∞’√‚"+this_object()->name()+"∞Â÷˜ "+ this_object()->query("master") +" £°\n"NOR,users());
...
"[47;34m  ◊˜’ﬂ [44;37m %-20s                    [47;34m  Ã÷¬€«¯  [37;44m%20s
[47;34m  ±ÍÃ‚ [44;37m %-70s
[47;34m   ±º‰ [44;37m %-70s[m  [36m\n------------------------------------------------------------------------------[m\n\n\n",
notes[num]["author"],name(),notes[num]["title"],ctime(notes[num]["time"]))
+ notes[num]["msg"] +
sprintf("\n\n\n°˘ Origin : »˝ΩÁ…Òª∞»™÷› ¶¥Û’æ < 210.34.120.6 6666 >
...
//by mon. 9/6/97
   ...
   argn=sscanf(arg, "%d %d %d", num, defmon, defyear);
        ...
        if(!arg || argn >3)
                return notify_fail("÷∏¡Ó∏Ò Ω£∫document <¡Ù—‘±‡∫≈>\n");
   ...
   if ( mon=="Null" ) {
     if (month>12 || month<1) return notify_fail("“™¥Êµƒ‘¬∑›≤ª¥Ê‘⁄£°\n");
     mon=strmon[month-1];
     }
   else switch (mon) {
