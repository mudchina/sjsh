...
string *factions=({"五庄观","大雪山","月宫","地府","东海龙宫","陷空山无底洞","蜀山剑牌","扬州城"});
...
void copy_npc(int sum)
{
	...
	if (t>=j) {
		...
		shout(HIW"【三界神话】魔界首领无天老祖大笑道：没想到如此轻易得手，真是天助我也，哈。哈。哈。下一个！\n"NOR);
...
string query_site()
{
	mapping sites=([
		...
		"峨嵋派":		({"/d/emei/hcaguangchang","/d/emei/hcazhengdian","/d/emei/huacangan","/d/emei/hcahoudian"}),
		"武当派":		({"/d/wudang/sanqingdian","/d/wudang/guangchang","/d/wudang/xuanwumen","/d/wudang/donglang2"}),
		...
		"扬州城":		({"/d/city/xidajie2","/d/city/beidajie1","/d/city/guangchang","/d/city/dongdajie2","/d/city/nandajie2"}),
	]);
...
void verdict_end()
{
	...
	if (count+1 != sizeof(factions)) {
		...
		shout(HIW"【三界神话】魔界首领无天老祖：他奶奶的，点子手硬，闪先！\n"NOR);
		shout(HIW"【"+HIM MUD_NAME+HIW"】暂时击退敌人，三界局势得到缓解，临时解除戒严状态！\n"NOR);
		...
		shout(HIW"【三界神话】魔界首领无天老祖仰天狂叫：我不服！！我不服！！！我不服！！！！\n"NOR);
		call_out("juedou",5);
	}
	LOGIN_D->set_madlock(0);
...
//			if (exp > 1000) user[i]->set("combat_exp",exp);
	...
	if (count+1 != sizeof(factions)) {
		...
		shout(HIW"【三界神话】魔界首领无天老祖冷哼一声：下一个！\n"NOR);
		shout(HIW"【"+HIM MUD_NAME+HIW"】敌人暂时撤退了，现在解除戒严状态！\n"NOR);
		call_out("disaster",10,count+1);
	}
	else {
		shout(HIR"【天地劫】"MAG"虽然各派奋力杀敌，无奈悬殊太大，还是败下阵来，一时间腥风血雨，三界一片混暗。\n"NOR);
		shout(HIW"【"+HIM MUD_NAME+HIW"】魔界首领大获全胜，三界一片黑暗，现在解除戒严状态！大家快逃命吧!\n"NOR);
	}
	LOGIN_D->set_madlock(0);
