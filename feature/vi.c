...
Add by great for more securityd in 1124 line.
 */
...
void start_edit(string file)                // 呼叫编辑器，开始编辑
{
        ...
        if (!(int)SECURITY_D->valid_write(file,this_player(),"write_file"))
        {
                write("你没有足够的读写权利。\n");  
                return;
        }
        ...
        log_file("edit/"+geteuid(this_player()),sprintf("VI %s %s\n",file,ctime(time())) );
...
if(!"/adm/daemons/securityd"->valid_write(str,pl,"_write_done"))
