...
obstacles = ob->query("obstacle/number");
        ...
        dntg = ob->query("dntg/number");
          ...
          gong = ob->query("12gong/number");
             if (gong == 0) line += " "HIY"黄金十二宫"NOR"：你还未曾闯过黄金十二宫\n";
                  else line+= HIY" 黄金十二宫"NOR"：你黄金十二宫已经过了"HIR+chinese_number(gong)+NOR"宫。\n";
             ct = ob->query("9sky/number"); 
                  if (ct == 0) line += " "RED"九　重　天"NOR"：你还闯过九重天。\n"; 
                 else line+= RED" 九　重　天"NOR"：你已经闯过了"HIR+chinese_number(ct)+NOR"重天。\n"; 
              rsg = ob->query("rsg_eaten");
             line+= HIW" 人参果记录"NOR"："HIR+chinese_number(rsg)+NOR"个。\n";    
        ...
        line += YEL"≡≡≡≡≡≡≡≡≡≡ "GRN"三界神话"HIY"·"HIG"泉州师院站"NOR YEL" ≡≡≡≡≡≡≡≡≡≡≡\n"NOR;
